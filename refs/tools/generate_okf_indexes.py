#!/usr/bin/env python3
"""Generate deterministic OKF discovery indexes for the WorkLore refs bundle."""
...
KNOWN_ACRONYMS = {"api": "API", "okf": "OKF", "prd": "PRD", "ui": "UI", "url": "URL"}
...
def humanize(value: str) -> str:
    ...
    for part in value.split():
        parts.append(KNOWN_ACRONYMS.get(part.casefold(), part.capitalize()))
    return " ".join(parts)
...
def visible_files(directory: Path) -> list[Path]:
    return sorted(
        [
            ...
            if child.is_file()
            and not child.name.startswith(".")
            and child.name not in RESERVED_MARKDOWN
        ],
        key=lambda p: p.name.casefold(),
    )
...
def bundle_directories() -> list[Path]:
    ...
    return sorted(
        directories, key=lambda p: p.relative_to(REFS).as_posix().casefold()
    )


def concept_entry(path: Path) -> str:
    return f"* [{humanize(path.stem)}]({path.name}) - WorkLore knowledge concept."


def resource_entry(path: Path, kind: str) -> str:
    return f"* [{path.name}]({path.name}) - WorkLore {kind} resource."
...
def expected_indexes() -> dict[Path, str]:
    ...
    title = str(bundle.get("title") or "WorkLore Knowledge Bundle")
    if not version:
...
def write_indexes(expected: dict[Path, str]) -> int:
    ...
    for path in REFS.rglob("index.md"):
        if path not in expected_paths and "__pycache__" not in path.parts:
            path.unlink()
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Fail if committed OKF indexes do not match deterministic output.",
    )
