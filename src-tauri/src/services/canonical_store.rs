...
pub fn database_path(vault_path: &Path) -> PathBuf { vault_path.join(DATABASE_RELATIVE_PATH) }
...
pub fn create_story_seed(vault_path: &Path, title: &str, summary: &str) -> ServiceResult<StorySeedRecord> {
    initialize(vault_path)?;
    let connection = open_connection(vault_path)?;
    ...
    let record = StorySeedRecord {
        ...
        summary: summary.trim().to_string(), status: "captured".to_string(), revision: 1,
    };
    connection.execute(
...
pub fn load_story_seed(vault_path: &Path, seed_id: &str) -> ServiceResult<StorySeedRecord> {
    ...
    connection.query_row(
        "SELECT seed_id,title,summary,status,revision FROM story_seeds WHERE seed_id=?1", [seed_id],
        |row| Ok(StorySeedRecord { seed_id: row.get(0)?, title: row.get(1)?, summary: row.get(2)?,
            status: row.get(3)?, revision: row.get(4)? })
    ).optional()?.ok_or_else(|| WorkLoreError::InvalidVault(format!("Story Seed {seed_id} was not found.")))
}

pub fn create_story(vault_path: &Path, title: &str, summary: &str) -> ServiceResult<CanonicalStoryRecord> {
    initialize(vault_path)?;
    let connection = open_connection(vault_path)?;
    let now = Utc::now().to_rfc3339();
    let record = CanonicalStoryRecord {
        story_id: format!("story_{}", Uuid::now_v7()), title: normalized_title(title, "Untitled story"),
        summary: summary.trim().to_string(), lifecycle_status: "active".to_string(),
        maturity: "developing".to_string(), revision: 1,
    };
    connection.execute(
...
pub fn create_evidence(vault_path: &Path, text: &str) -> ServiceResult<String> {
    initialize(vault_path)?;
    let connection = open_connection(vault_path)?;
    let id = format!("evidence_{}", Uuid::now_v7()); let now = Utc::now().to_rfc3339();
    connection.execute(
...
pub fn create_inspiration(vault_path: &Path, title: &str) -> ServiceResult<String> {
    initialize(vault_path)?;
    let connection = open_connection(vault_path)?;
    let id = format!("inspiration_{}", Uuid::now_v7()); let now = Utc::now().to_rfc3339();
    connection.execute(
...
pub fn create_target_context(vault_path: &Path, context_type: &str, title: &str) -> ServiceResult<String> {
    initialize(vault_path)?;
    let connection = open_connection(vault_path)?;
    let id = format!("target_{}", Uuid::now_v7()); let now = Utc::now().to_rfc3339();
    connection.execute(
...
pub fn migrate_prototype(vault_path: &Path) -> ServiceResult<PrototypeMigrationSummary> {
    ...
    let mut connection = open_connection(vault_path)?; migrate_schema(&mut connection)?;
    let tx = connection.transaction()?; let mut summary = PrototypeMigrationSummary::default();
    for source in &sources { migrate_source(&tx, source)?; summary.sources += 1;
        if source.source_type == SourceType::JobDescription { migrate_job_target(&tx,source)?; summary.target_contexts += 1; } }
    for role in &roles { migrate_role(&tx,role)?; summary.roles += 1; }
    for candidate in &candidates { migrate_candidate(&tx,candidate)?; summary.seeds += 1; }
    for story in &stories { summary.evidence += migrate_story(&tx,story)?; summary.stories += 1; }
    for interview in &interviews { migrate_interview(&tx,interview)?; summary.interviews += 1; }
    tx.commit()?; Ok(summary)
}
...
fn open_connection(vault_path: &Path) -> ServiceResult<Connection> {
    ...
    connection.pragma_update(None,"foreign_keys","ON")?; connection.busy_timeout(Duration::from_secs(5))?;
    Ok(connection)
}

fn migrate_schema(connection: &mut Connection) -> ServiceResult<()> {
    connection.execute_batch("CREATE TABLE IF NOT EXISTS schema_migrations(
        version INTEGER PRIMARY KEY,name TEXT NOT NULL,applied_at TEXT NOT NULL);")?;
    let version: i64 = connection.query_row("SELECT COALESCE(MAX(version),0) FROM schema_migrations",[],|r| r.get(0))?;
    if version > CURRENT_SCHEMA_VERSION { return Err(WorkLoreError::InvalidVault(format!(
        "Canonical database schema version {version} is newer than this WorkLore build supports."))); }
    if version == 0 { let tx = connection.transaction()?; tx.execute_batch(SCHEMA_V1)?;
        tx.execute("INSERT INTO schema_migrations(version,name,applied_at) VALUES (1,'professional_memory_v1',?1)",
            [Utc::now().to_rfc3339()])?; tx.commit()?; }
    Ok(())
}

fn migrate_source(tx: &Transaction<'_>, source: &SourceDocument) -> ServiceResult<()> {
    validate_relative(&source.stored_path)?; let size = i64::try_from(source.byte_size)
        .map_err(|_| WorkLoreError::InvalidVault(format!("Source {} is too large.",source.source_id)))?;
    tx.execute(
...
fn migrate_job_target(tx: &Transaction<'_>, source: &SourceDocument) -> ServiceResult<()> {
    let id = transformed_id(tx,"job_description_source",&source.source_id,"target_context","target")?;
    tx.execute(
...
fn migrate_role(tx: &Transaction<'_>, role: &RoleRecord) -> ServiceResult<()> {
    ...
    for source in &role.source_ids { relationship(tx,"role",&role.role_id,"role_source","source",source)?; }
    for story in &role.story_ids { relationship(tx,"story",story,"story_role","role",&role.role_id)?; }
    Ok(())
}

fn migrate_candidate(tx: &Transaction<'_>, candidate: &StoryCandidate) -> ServiceResult<String> {
    let id = transformed_id(tx,"story_candidate",&candidate.candidate_id,"story_seed","seed")?;
    tx.execute(
        "INSERT INTO story_seeds(seed_id,title,summary,status,provenance_json,created_at,updated_at,revision)
    ...
    for source in &candidate.source_refs { relationship(tx,"story_seed",&id,"seed_source","source",&source.source_id)?; }
    for role in &candidate.context.role_ids { relationship(tx,"story_seed",&id,"seed_role_context","role",role)?; }
    Ok(id)
}
...
fn migrate_story(tx: &Transaction<'_>, story: &StoryRecord) -> ServiceResult<usize> {
    ...
    for role in &story.role_ids { relationship(tx,"story",&story.story_id,"story_role","role",role)?; }
    let mut count=0;
    for evidence in &story.evidence {
        ...
        relationship(tx,"story",&story.story_id,"story_evidence","evidence",&evidence.evidence_id)?; count+=1;
    }
    Ok(count)
}

fn migrate_interview(tx: &Transaction<'_>, interview: &InterviewSession) -> ServiceResult<()> {
    let seed = interview.candidate_ids.first().map(|id| transformed_id(tx,"story_candidate",id,"story_seed","seed")).transpose()?;
    tx.execute(
...
fn relationship(tx:&Transaction<'_>,from_type:&str,from_id:&str,kind:&str,to_type:&str,to_id:&str)->ServiceResult<()> {
    tx.execute("INSERT OR IGNORE INTO record_relationships(relationship_id,from_type,from_id,relationship_type,to_type,to_id,
        ...
        from_type,from_id,kind,to_type,to_id,json!({"creationActor":"migration"}).to_string(),Utc::now().to_rfc3339()])?; Ok(())
}

fn preserve_id(tx:&Transaction<'_>,legacy_type:&str,id:&str,canonical_type:&str)->ServiceResult<()> {
    lineage(tx,legacy_type,id,canonical_type,id,json!({"idPreserved":true}).to_string())
}

fn transformed_id(tx:&Transaction<'_>,legacy_type:&str,legacy_id:&str,canonical_type:&str,prefix:&str)->ServiceResult<String> {
    ...
    let id=format!("{prefix}_{}",Uuid::now_v7()); lineage(tx,legacy_type,legacy_id,canonical_type,&id,
        json!({"idPreserved":false}).to_string())?; Ok(id)
}

fn lineage(tx:&Transaction<'_>,legacy_type:&str,legacy_id:&str,canonical_type:&str,canonical_id:&str,details:String)->ServiceResult<()> {
    let now=Utc::now().to_rfc3339(); let inserted=tx.execute(
    ...
    if inserted>0 { tx.execute("INSERT INTO audit_events(audit_id,event_type,record_type,record_id,actor,details_json,occurred_at)
        VALUES (?1,'migration',?2,?3,'migration',?4,?5)",params![format!("audit_{}",Uuid::now_v7()),canonical_type,canonical_id,
            json!({"legacyType":legacy_type,"legacyId":legacy_id}).to_string(),now])?; } Ok(())
}

fn read_records<T:DeserializeOwned>(directory:&Path)->ServiceResult<Vec<T>> {
    if !directory.exists(){return Ok(Vec::new());} let mut out=Vec::new();
    for entry in fs::read_dir(directory)? { let entry=entry?; if entry.file_type()?.is_file()
        && entry.path().extension().and_then(|v|v.to_str())==Some("json") { out.push(serde_json::from_slice(&fs::read(entry.path())?)?); } }
    Ok(out)
}

fn validate_relative(value:&str)->ServiceResult<()> {
    let v=value.replace('\\',"/"); let b=v.as_bytes(); let drive=b.len()>=3 && b[1]==b':' && b[2]==b'/';
    if v.is_empty()||v.starts_with('/')||drive||v.split('/').any(|p|p=="..") { Err(WorkLoreError::InvalidPath) } else { Ok(()) }
}

fn enum_text<T:Serialize>(value:&T)->ServiceResult<String> { match serde_json::to_value(value)? {
    serde_json::Value::String(v)=>Ok(v), _=>Err(WorkLoreError::InvalidVault("Expected string-backed domain enum.".to_string())) } }
fn normalized_title(value:&str,fallback:&str)->String { let v=value.trim(); if v.is_empty(){fallback.to_string()}else{v.to_string()} }
fn compact(value:&str,max:usize)->String { let v=value.split_whitespace().collect::<Vec<_>>().join(" ");
    if v.chars().count()<=max {v} else {let mut s=v.chars().take(max.saturating_sub(3)).collect::<String>();s.push_str("...");s} }
fn candidate_status(status:CandidateStatus)->&'static str { match status {
    ...
    CandidateStatus::ConvertedToStory=>"converted", CandidateStatus::Ignored|CandidateStatus::Unsupported=>"dismissed",
    CandidateStatus::Merged|CandidateStatus::Split=>"archived" } }
fn story_states(status:StoryStatus)->(&'static str,&'static str) { match status {
    StoryStatus::Draft|StoryStatus::Interviewing=>("active","developing"),
    StoryStatus::ReadyForReview|StoryStatus::Validated=>("active","evidence_rich"), StoryStatus::Finalized=>("active","ready_to_use"),
    StoryStatus::Archived=>("archived","ready_to_use") } }
fn ensure_vault(vault_path:&Path)->ServiceResult<()> { if vault_path.join("vault.json").is_file(){Ok(())}else{Err(WorkLoreError::NotAVault)} }

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{domain::{candidates::{CandidateContext,CandidateSourceRef,CandidateType},models::{ContentHash,ExtractionState,
        ExtractionStatus,PrivacyScanState,PrivacyScanStatus,SourceProvenance},stories::{EvidenceType,StoryContent,StoryDisclosure,
        StoryEvidence,StoryPrivacyScan,StoryType}},io_utils::write_json_atomic,services::vault_service};

    fn vault()->PathBuf { let p=std::env::temp_dir().join(format!("worklore-sqlite-{}",Uuid::now_v7()));
        vault_service::create_vault(&p,"Test").expect("create vault"); p }

    #[test] fn initializes_database(){let p=vault();assert!(database_path(&p).is_file());assert_eq!(schema_version(&p).unwrap(),1);fs::remove_dir_all(p).unwrap();}
    #[test] fn seed_survives_reopen_without_resume(){let p=vault();let a=create_story_seed(&p,"Memory","fragment").unwrap();
        initialize_and_migrate(&p).unwrap();assert_eq!(load_story_seed(&p,&a.seed_id).unwrap(),a);fs::remove_dir_all(p).unwrap();}
    #[test] fn story_does_not_require_role(){let p=vault();let a=create_story(&p,"Direct","No resume").unwrap();
        assert_eq!(load_story(&p,&a.story_id).unwrap(),a);let c=open_connection(&p).unwrap();let n:i64=c.query_row(
        "SELECT COUNT(*) FROM record_relationships WHERE from_id=?1 AND relationship_type='story_role'",[&a.story_id],|r|r.get(0)).unwrap();
        assert_eq!(n,0);drop(c);fs::remove_dir_all(p).unwrap();}
    #[test] fn semantic_roles_are_separate(){let p=vault();create_evidence(&p,"fact").unwrap();create_inspiration(&p,"article").unwrap();
        create_target_context(&p,"job_description","job").unwrap();let c=open_connection(&p).unwrap();
        let e:i64=c.query_row("SELECT COUNT(*) FROM evidence_records",[],|r|r.get(0)).unwrap();let i:i64=c.query_row("SELECT COUNT(*) FROM inspirations",[],|r|r.get(0)).unwrap();
        let t:i64=c.query_row("SELECT COUNT(*) FROM target_contexts",[],|r|r.get(0)).unwrap();assert_eq!((e,i,t),(1,1,1));drop(c);fs::remove_dir_all(p).unwrap();}

    #[test] fn migration_preserves_ids_and_uses_new_seed_id(){let p=vault();let now=Utc::now().to_rfc3339();
        let source=SourceDocument{schema_version:1,source_id:"source_synthetic".into(),source_type:SourceType::Resume,
            display_name:"Synthetic Resume".into(),stored_path:"sources/resumes/synthetic.txt".into(),original_file_name:"synthetic.txt".into(),
            original_path_hint:Some("C:\\private\\synthetic.txt".into()),media_type:Some("text/plain".into()),byte_size:12,
            content_hash:ContentHash{algorithm:"sha256".into(),value:"0".repeat(64)},imported_at:now.clone(),updated_at:now.clone(),
            extraction:ExtractionState{status:ExtractionStatus::Complete,extractor_version:Some("test".into()),text_path:None,character_count:12,warnings:vec![],error:None},
            privacy_scan:PrivacyScanState{status:PrivacyScanStatus::Complete,scan_version:Some("test".into()),scanned_at:Some(now.clone()),review_item_ids:vec![]},
            provenance:SourceProvenance{import_method:"file_copy".into(),parent_source_id:None,notes:String::new()},tags:vec![]};
        write_json_atomic(&p.join("sources/metadata/source_synthetic.json"),&source).unwrap();
        let candidate=StoryCandidate{schema_version:1,candidate_id:"candidate_synthetic".into(),source_refs:vec![CandidateSourceRef{source_id:source.source_id.clone(),fragment_id:"f1".into(),captured_text:"Improved workflow".into()}],
            candidate_type:CandidateType::ResumeClaim,status:CandidateStatus::New,claim:"Improved workflow".into(),context:CandidateContext{role_ids:vec![],entity_ids:vec![],skills:vec![],tools:vec![],metrics:vec![],surrounding_heading:None},
            missing_fields:vec![],possible_existing_story_ids:vec![],merged_into_id:None,split_into_ids:vec![],created_at:now.clone(),updated_at:now.clone(),revision:1};
        write_json_atomic(&p.join("candidates/candidate_synthetic.json"),&candidate).unwrap();
        let story=StoryRecord{schema_version:1,story_id:"story_synthetic".into(),title:"Synthetic Story".into(),status:StoryStatus::Validated,story_type:StoryType::ProcessChange,
            role_ids:vec![],entity_ids:vec![],related_story_ids:vec![],job_requirement_ids:vec![],content:StoryContent{summary:"Improved workflow".into(),situation:String::new(),problem_or_opportunity:String::new(),responsibilities:vec![],constraints:vec![],actions_and_decisions:vec![],alternatives_considered:vec![],tools_and_systems:vec![],stakeholders:vec![],outcomes:vec![],metrics:vec![],lessons_learned:vec![],operating_philosophy:vec![],reusable_themes:vec![],skills_demonstrated:vec![]},claims:vec![],
            evidence:vec![StoryEvidence{evidence_id:"evidence_synthetic".into(),evidence_type:EvidenceType::SourceFragment,source_id:Some(source.source_id.clone()),locator:"f1".into(),captured_text:"Improved workflow".into(),captured_at:Some(now.clone())}],
            disclosure:StoryDisclosure{default_public_mode:"stable_tokens".into(),review_required:true,notes:String::new()},privacy_scan:StoryPrivacyScan{status:PrivacyScanStatus::Complete,content_revision:1,scanned_at:Some(now.clone()),review_item_ids:vec![]},tags:vec![],created_at:now.clone(),updated_at:now,revision:1};
        write_json_atomic(&p.join("stories/story_synthetic.json"),&story).unwrap();migrate_prototype(&p).unwrap();let c=open_connection(&p).unwrap();
        let source_n:i64=c.query_row("SELECT COUNT(*) FROM sources WHERE source_id='source_synthetic'",[],|r|r.get(0)).unwrap();let story_n:i64=c.query_row("SELECT COUNT(*) FROM stories WHERE story_id='story_synthetic'",[],|r|r.get(0)).unwrap();
        ...
        let path_col:i64=c.query_row("SELECT COUNT(*) FROM pragma_table_info('sources') WHERE name='original_path_hint'",[],|r|r.get(0)).unwrap();
        assert_eq!((source_n,story_n),(1,1));assert!(seed.starts_with("seed_"));assert_ne!(seed,candidate.candidate_id);assert_eq!(path_col,0);
        assert!(p.join("sources/metadata/source_synthetic.json").is_file());assert!(p.join("stories/story_synthetic.json").is_file());drop(c);fs::remove_dir_all(p).unwrap();}
}
